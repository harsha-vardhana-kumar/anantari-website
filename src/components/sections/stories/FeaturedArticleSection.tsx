    ...
    <section className="py-16 px-6 bg-ivory-dark">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-8 items-stretch">
        ...
        <motion.div
          ...
          className="lg:col-span-3 bg-white border border-navy/[0.08] shadow-sm hover:shadow-xl rounded-3xl overflow-hidden group hover:border-brand/30 hover:shadow-[0_0_60px_rgba(238,103,13,0.08)] transition-all duration-300"
        >
          {/* Top stats area */}
          <div className="bg-navy p-6 md:p-8 flex justify-between items-center divide-x divide-white/10">
            {featuredArticle.stats.map((stat, i) => (
              <div
                key={stat.label}
                className="flex-1 text-center px-4 first:pl-0 last:pr-0"
              >
                <p className={`font-cormorant text-2xl md:text-3xl font-bold ${statColors[i]}`}>{stat.value}</p>
                <p className="text-[10px] md:text-xs text-ivory/50 mt-1.5 uppercase tracking-widest font-semibold">{stat.label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="p-8">
            ...
            {/* Title */}
            <h2 className="font-cormorant text-2xl md:text-3xl font-bold text-navy leading-tight hover:text-brand transition-colors cursor-pointer">
              {featuredArticle.title}
            </h2>

            {/* Subtitle */}
            <p className="text-navy/60 text-sm leading-relaxed mt-4 line-clamp-3">
            ...
            {/* Author + date row */}
            <div className="mt-6 pt-6 border-t border-navy/10 flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-navy">{featuredArticle.author.name}</p>
                  <p className="text-xs text-navy/50">{featuredArticle.author.role}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs text-navy/30">{featuredArticle.date}</span>
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          {secondaryArticles.map((article, idx) => (
            <motion.div
              ...
              className="bg-white border border-navy/[0.08] shadow-sm hover:shadow-lg rounded-3xl p-6 flex-1 flex flex-col group hover:border-brand/30 hover:shadow-[0_0_30px_rgba(238,103,13,0.06)] transition-all"
            >
              {/* Top row */}
              <div className="flex justify-between items-center">
                <span className="bg-navy/5 border border-navy/10 text-navy/50 text-xs rounded-full px-3 py-1">
                  {article.category}
                </span>
              ...
              {/* Title */}
              <h3 className="font-cormorant text-lg font-bold text-navy mt-3 leading-tight group-hover:text-brand transition-colors flex-1">
                {article.title}
              </h3>
              ...
              <div className="flex gap-2 flex-wrap mt-3">
                {article.tags.slice(0, 2).map((tag) => (
                  <span key={tag} className="bg-navy/5 border border-navy/10 text-navy/40 text-xs rounded-full px-3 py-1">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-navy/10 flex items-center gap-3">
                ...
                <span className="text-xs text-navy/30 ml-auto">{article.date}</span>
              </div>
